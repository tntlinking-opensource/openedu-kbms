	...
	function isSameActivity(other){
		return this.canMergeWith(other)&&(this.vaildWeeks==other.vaildWeeks&&this.teacherId==other.teacherId);
	}
	/**
	 *合并相同的教学活动
	 *same [teacherId,courseId,roomId] can merge
	 * edit by zhihe
	 * same [vaildWeeks,courseId,roomId] can merge
	 */
	function canMergeWith(other){
	   //return (this.teacherId==other.teacherId&&this.courseId==other.courseId&&this.courseName==other.courseName&&this.roomId==other.roomId);
		return (this.taskId==other.taskId&&this.vaildWeeks==other.vaildWeeks&&this.courseId==other.courseId&&this.courseName==other.courseName&&this.roomId==other.roomId);
	}
    ...
    function marshalByTeacherCourse(index,from,startWeek,endWeek){
    	...
    	if(this.activities[index].length==1){  
    	  ...
    	  if (tname == "") {
    		  return cname +delimiter+"(" +this.activities[index][0].adjustClone(this.endAtSat,validStart,false).marshal(from,startWeek,endWeek)+")";
    	  }
    	  return cname +delimiter+ tname +delimiter+"(" +this.activities[index][0].adjustClone(this.endAtSat,validStart,false).marshal(from,startWeek,endWeek)+")";
    	}
    	else {
         	var marshalString="";
    	    ...
    	    for(var i=1;i<this.activities[index].length;i++){
    	       ...
    	       for(var j=0;j< tempActivities.length;j++){
    	           if(this.activities[index][i].canMergeWith(tempActivities[j])){
	    	           ...
	    	           // 合并显示教师
    	        	   tempActivities[j].teacherName += " " + this.activities[index][i].teacherName;
	    	           merged=true;
    	       ...
    	       if(!merged){
    	           tempActivities[tempActivities.length] = this.activities[index][i].adjustClone(this.endAtSat,validStart,true);
    	       }
    	    }
    	    ...
    	    for(var m=0;m<tempActivities.length;m++){
                ...
                if(courseName!==null) {
                	if (teacherName == "") {
	                	...
	                	marshalString+=delimiter+ courseName + delimiter + teacherName;/*alert(courseName);*/
                	}
                }
    ...
    * course table,and adminClass's course table,even major's .
    ************************************************************************/
