	...
	function abbreviate(available) {
		...
		for (var i = 0; i < 7; i++) {
			...
			for (var j = 0; j < maxUnit; j++) {
				dayAvalible += available.charAt(j * Weeks.length + i);
			}
			//available.substr(i * maxUnit,maxUnit);
 ...
 colors["0"]="yellow";
     function changeAvailTime(elem){         
         ...
         var firstId=elem.id;
         var weekId=Math.floor(firstId/Weeks.length);
         for(var i=0;i<num;i++){
             ...
             firstId = parseInt(firstId) + Weeks.length;
             if(Math.floor(firstId/Weeks.length)>=unitsPerDay) break;
         }
      }
     ...
     function getRemark(){
        return document.getElementById("stBz").value;
     }
     function inverse(){
