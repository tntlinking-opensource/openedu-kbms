...
<html>
	...
	<body style="overflow-x: hidden;background-color:#f4f8fd;">
		<div class="aichat-header">
			<div class="aichat-header-menu">
				<div class="header-menu-item  <#if dhlx?exists && dhlx == '1'>act</#if>" onclick="location.href='${base}/aichat/t_ai_znt!aiAgent.action?id=${entity.id!}&dhlx=1'">
					<span>AI问答</span>
				</div>
				<div class="header-menu-item  <#if dhlx?exists && dhlx == '2'>act</#if>" onclick="location.href='${base}/aichat/t_ai_znt!aiAgent.action?id=${entity.id!}&dhlx=2'">
					<span>场景对话</span>
				</div>
				<div class="header-menu-item  <#if dhlx?exists && dhlx == '3'>act</#if>">
					<span>数据智能</span>
				</div>
			</div>
		</div>
		<div class="aichat-main">
			<div class="aichat-main-left">
				...
				<div class="aichat-main-left-tp">
					<input style="" type="button" name="search_btn" onclick="location.href='${base}/aichat/t_ai_znt!aiAgent.action?id=${entity.id!}'" class="search_btn mfbmbtn" value=" + 新 建 对 话 " onclick="">
				</div>
				...
				<div class="aichat-main-left-bdy">
					<div class="chatgroup-time-content">
					    <#if dhlsList?exists>
					        <#list dhlsList as obj >
					            <div class="chatgroup-item <#if dhlsid?exists && obj.id == dhlsid>chatgroup-item-checkls</#if>" onclick="location.href='${base}/aichat/t_ai_znt!aiAgent.action?id=${id!}&dhlsid=${obj.id!}'" style="" title="${obj.biaot!}">
					                ${obj.biaot!}
			...
			<div class="aichat-main-right">
				<div class="chat-box">
					...
					<div class="chat-box-index">
									...
									<#if obj.role == '1'>
										<div class="message bot">
											<div class="msgbot-icon">
												<img src="${base}/mainface/dapingdemo/studyNewUi/images/kc_pj.png" style="width:1.5vw;">
											</div>
											<#if obj.skgc?exists && obj.skgc != "" && obj.skgc?trim != "">
												<div class="bot-think">
													<div class="div_p">${obj.skgc!}</div>
												</div>
											</#if>
						...
						<#else>
							<div class="znt_clas">
								<div class="chat-box-index-1" style="margin-top:5vh;">
								...
								<div class="chat-box-index-3" style="display: flex;justify-content: center;height:8vh;">
									<div style="width:50%;">
										<#if entity.jies?exists>
											<#if entity.jies?length gt 95>
												${entity?if_exists.jies.replaceAll("\n", "<br/>")?substring(0,95)!}...
											<#else>
								...
								<div class="chat-box-index-4" style="justify-content: center;">
									<#if zntMap?exists>	
										<div class="button-container" style="width: 70%;">
											<#list zntMap?keys as key>
											    <button class="checkbtn <#if key_index ==0>active</#if> ">${key!}</button>
										    </#list>
										</div>
									</#if>
									<div style="display: flex;justify-content: center;">
										<#if zntMap?exists>	
											<#list zntMap?keys as key>
												<div class="chat-box-card <#if key_index ==0>container_show<#else>container_hidden</#if>">
												<div class="container " >
												    <#if zntMap[key]?exists && zntMap[key]?size gt 0>
													    <#list zntMap[key] as obj>
													    <div class="card" onclick="window.open('${base}/aichat/t_ai_znt!aiAgent.action?id=${obj.id!}')">
													      <div class="card-icon" >
	...
	<script>
	     $(document).ready(function() {
	        const $buttons = $(".checkbtn");
	        const $chatcard = $(".chat-box-card");
	        $buttons.on('click', function() {
	            $buttons.removeClass('active');
	            $(this).addClass('active');
	            const index = $buttons.index(this);
	            $chatcard.removeClass('container_show');
	            $chatcard.addClass('container_hidden');
	            $chatcard.eq(index).addClass('container_show');
	        });
	    });
	</script>
	<script>
		...
		$(document).ready(function() {
		    ...
		    function updateReasoning(msg) {
		    	...
		    	if(typeof(chatbot) === 'undefined'){
		    		 chatBox.append(
					  '<div id="message_'+ msg.id +'" class="message bot">' +
					  	'<div class="msgbot-icon"><img src="${base}/mainface/dapingdemo/studyNewUi/images/kc_pj.png" style="width:1.5vw;"></div>'+
		        ...
		        if (msg.reasoning_content && $.trim(msg.content) !== "") {
		    ...
		    function insertYhdhls(biaot){
		    	...
		    	$.post("${base}/aichat/t_ai_znt!insertYhdhls.action",parm,function(res){
		    		...
		    		$(".chatgroup-time-content").prepend(
					    '<div class="chatgroup-item chatgroup-item-checkls" onclick="location.href=\'${base}/aichat/t_ai_znt!aiAgent.action?id=${id!}&dhlsid=' + dhlsid + '\'">' +
					    biaot +
		    ...
		    if (confirm("确定要删除此项吗？")) {
		       $.post("${base}/aichat/t_ai_yhdhls!delete.action",{checks:id},function(res){
					...
					layer.msg(res, {
						...
						location.href="${base}/aichat/t_ai_znt!aiAgent.action?id=${entity.id!}&dhlsid=" + dhlsid;
					});
			...
			if (confirm("确定要清除所有对话？")) {
				...
				$.post("${base}/aichat/t_ai_yhdhls!delAll.action",{zntid:zntid},function(res){
		       		layer.msg(res, {
						...
						location.href="${base}/aichat/t_ai_znt!aiAgent.action?id=${entity.id!}";
					});
		       })
			}
		}
